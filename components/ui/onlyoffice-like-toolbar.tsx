    ...
    <div className="w-full flex items-center bg-[#23272b] text-white h-10 border-b border-neutral-800 select-none">
      <div className="flex gap-1 px-2">
        {tabs.map((tab) => (
          <button
            key={tab.label}
            className={`px-3 py-1 text-sm font-medium transition-colors duration-100 outline-none ${
              tab.active || activeTab === tab.label
                ? "bg-[#23272b] text-white border-b-2 border-violet-500"
                : "bg-transparent text-neutral-300 hover:bg-neutral-700 border-b-2 border-transparent"
            }`}
